...
// Kuberhealthy using the client package. The checker uses environment variables
// provided to the pod by Kuberhealthy to know where and how to report results.
func main() {
	...
	log.Printf("reporting to %s with run UUID %s", reportingURL, runUUID)
        ...
        if os.Getenv("FAIL") == "true" {
                if err := checkclient.ReportFailure([]string{"example failure"}); err != nil {
